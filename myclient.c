...
int get_port_string(char *str, char *ip,int b5,int b6){
    int i = 0;
    char ip_temp[1024];
    strcpy(ip_temp, ip);
 ...
 sprintf(str, "PORT %s,%d,%d", ip_temp,b5,b6);
    return 1;}
...
int get_command(char *command){
	...
	while(check < 0){
    	...
    	if(get_user_input(command) < 0){
    		printf("Eroare, incarca din nou......\n");
            bzero(command, (int)sizeof(command));
    		continue;}
     if(strlen(command) < 2){
            printf("Imput detetectat...\n");
            bzero(command, (int)sizeof(command));
            continue;}  
        taie(command);
        strcpy(copy, command);
   if(check_command(copy) < 0){
    	...
    	if((strcmp(str, "ls") == 0) || (strcmp(str, "get") == 0) || (strcmp(str, "put") == 0) || (strcmp(str, "quit") == 0)){
    		check = 1;
            if(strcmp(str, "ls") == 0){val = 1;}
            else if(strcmp(str, "get") == 0){val = 2;}
            else if(strcmp(str, "put") == 0){val = 3;}
            else if(strcmp(str, "quit") == 0){val = 4;}
    	}else{
    		printf("Coamnda incorecta.\n........\n");
            bzero(command,strlen(command));
           	bzero(copy,sizeof(copy));
    		continue;} }
	return val;}
int convert(uint16_t port, int *b5, int *b6){
    int i = 0;
    int x = 1;
    *b5 = 0;
    *b6 = 0;
    ...
    for(i = 8; i< 16; i++){
        ...
        *b5 = ((*b5)|(temp));
        x = x << 1;   }
    return 1;}
int g_ip_port(int fd, char *ip, int *port){
...
int do_get(int controlfd, int datafd, char *input){
    ...
    if(get_filename(input, filename) < 0){
        printf("Nume de fisier nedetectat...\n");
        char send[1024];
        sprintf(send, "SKIP");
...
int main(int argc, char **argv){
	int server_port, controlfd, listenfd, datafd, code,b5,b6, x;
    uint16_t port;
	struct sockaddr_in servaddr, data_addr;
	char command[1024], ip[50], str[MAXLINE+1];
if(argc != 3){
		printf("Invalid numar de argumente...\n");
...
listen(listenfd, LISTENQ);
    g_ip_port(controlfd, ip, (int *)&x);
    printf("x: %d\n", x);
    printf("ip: %s\n", ip);
    g_ip_port(listenfd, str, (int *)&port);
       printf("Port: %d, str: %s\n",  port, str);
    convert(port, &b5, &b6);
    while(1){
        bzero(command, strlen(command));
